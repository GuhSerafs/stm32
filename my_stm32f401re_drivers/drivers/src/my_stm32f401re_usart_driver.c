...
void USART_PeripheralControl(USART_RegDef_t *pUSARTx, uint8_t EnOrDis) {
	if (EnOrDis) {
		pUSARTx->CR1 |= (1 << USART_CR1_UE); // Habilita o USART Enable
	} else {
		pUSARTx->CR1 &= ~(1 << USART_CR1_UE); // Desativa o USART Enable
	}
}
uint8_t USART_GetFlagStatus(USART_RegDef_t *pUSARTx, uint32_t FlagName) {
	uint8_t status = ((pUSARTx->SR >> FlagName) & 0x1);
	return status;
}
